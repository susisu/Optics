...
class CommandBase {
    ...
    /**
     * Generates help text of the command.
     * @param {string} cmdName The command name used when it is invoked.
...
class Command extends CommandBase {
    ...
    /**
     * Generates help text of the command.
     * @param {string} cmdName The command name used when the command is invoked.
    ...
    generateHelpText(cmdName) {
        ...
        let argsList = this.args.map(arg => arg.toPlaceholder()).join(" ");
        let optDescs = this.opts.map(opt => [
            opt.toDescriptor(),
            opt.desc
        ]);
        let usageText = `  Usage: ${cmdName}`
            + (this.opts.length > 0 ? " [options]" : "")
            + (this.args.length > 0 ? ` ${argsList}` : "")
            + "\n\n";
        let optsText = this.opts.length > 0
            ? `  Options:\n${help.formatHelpText(4, optDescs)}\n`
            : "";
        return "\n"
            + `  ${this.desc}\n\n`
            + usageText
